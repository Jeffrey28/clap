...
class predict():
    ...
    def check_collision(self, fp):
        ...
        return False #if try did not work, it is not assured whether the path has collision or not, should return false
...
class JunctionPredict():
    ...
    def ctrv_prediction_obstacle(self, ob, max_prediction_time, delta_t): # we should do prediciton in driving space
...
class __ngLanePredict():
    ...
    def found_closest_obstacles(self):
        ...
        for lane_id in len(dynamic_map.mmap.lanes):
            front_obs.extend(dynamic_map.mmap.lanes[lane_id].front_vehicles)
            print("prediction:current lane", lane_id)
            print("find front obs:", len(dynamic_map.mmap.lanes[lane_id].front_vehicles))
